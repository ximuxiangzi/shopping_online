        ...
        if (valid) {
          ...
          Login(data).then((res) => {
            if (res.code !== 200) {
              ...
              this.$store.commit("Options/my_userInfo", res.data.userInfo);
              ...
              let dynamicList = res.data.userInfo.roles[0].treeNodes;
              console.log(dynamicList);
              localStorage.setItem("statusMenu", "0");
              this.$store.commit("Options/menuData", dynamicList);

              this.$store.commit("Options/setPermission", hasData);
